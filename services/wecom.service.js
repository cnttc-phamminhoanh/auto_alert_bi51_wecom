...
const { getFlowWarningStats, getWeComIDsFromDB } = require('../repositories/report.messageredorgon');
const { splitIntoGroups } = require('../utils/report-file.util');

const HARDCODED_IDS = [
    '377208278', '857875232', '762589598', '979507463'
];
 ...
 */
async function generateFlowMessage(db) {
    const stats = await getFlowWarningStats(db);
 ...
 */
async function sendFlowStatsMessage(webhookUrl, db) {
    try {
        const message = await generateFlowMessage(db);
...
async function getWeComIDs(db) {
    const hardcodedIds = HARDCODED_IDS.filter(id => id && id.trim() !== '');
    const dbIds = await getWeComIDsFromDB(db);
    const allIds = [...hardcodedIds];
    dbIds.forEach(id => { if (!allIds.includes(id)) allIds.push(id); });
    return allIds;
}

// 🆕 Gửi tin nhắn tag người liên quan cảnh báo (đổi tên tránh trùng sendMentionMessage)
async function sendWarningMentionMessage(webhookUrl, db, customMessage = null) {
    try {
        const ids = await getWeComIDs(db);
        if (ids.length === 0) {
