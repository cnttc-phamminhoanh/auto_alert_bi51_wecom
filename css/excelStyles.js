...
function getWarningStyle(warningValue) {
  ...
  if (
    warningStr.includes("61") ||
    warningStr.includes("90") ||
    warningStr.includes("120")
  ) {
...
function createBorder(config) {
  return {
    top: {
      style: config.top?.style || "thin",
      color: { argb: config.top?.color || "FFD0D0D0" },
    },
    left: {
      style: config.left?.style || "thin",
      color: { argb: config.left?.color || "FFD0D0D0" },
    },
    bottom: {
      style: config.bottom?.style || "thin",
      color: { argb: config.bottom?.color || "FFD0D0D0" },
    },
    right: {
      style: config.right?.style || "thin",
      color: { argb: config.right?.color || "FFD0D0D0" },
    },
  };
}
